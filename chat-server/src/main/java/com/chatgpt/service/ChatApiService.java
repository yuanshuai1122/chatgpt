...
import com.chatgpt.config.OkHttpClientSingleton;
...
public class ChatApiService {
  ...
  public ResponseResult<String> chatSign(ChatProcess process) {
    ...
    wrapper.lambda().eq(ChatUserKey::getUserKey, process.getChatKey());
    ChatUserKey chatUserKey = chatUserKeyMapper.selectOne(wrapper);
    if (null == chatUserKey) {
      return new ResponseResult<>(ResultCode.KEY_NOT_FOUND.getCode(), ResultCode.KEY_NOT_FOUND.getMsg());
    }
    if (chatUserKey.getRemainTimes() <= 0) {
      return new ResponseResult<>(ResultCode.KEY_NOT_TIMES.getCode(), ResultCode.KEY_NOT_TIMES.getMsg());
    }
    if (chatUserKey.getExpireTime().getTime() < System.currentTimeMillis()) {
      return new ResponseResult<>(ResultCode.KEY_NOT_DATE.getCode(), ResultCode.KEY_NOT_DATE.getMsg());
    }
    if (!chatUserKey.getActiveStatus().equals(ChatKeyStatusEnum.ACTIVED.getStatus())) {
      return new ResponseResult<>(ResultCode.KEY_NOT_ACTIVE.getCode(), ResultCode.KEY_NOT_ACTIVE.getMsg());
    }
    ...
    // 写日志数据库
    ChatSuccessLog chatSuccessLog = new ChatSuccessLog(null, chatUserKey.getId(), ChatRoleEnum.USER.getRole(), ValueUtils.getMessageUUID(), signKey, process.getPrompt(), new Date());
    asyncTask.setChatLog(chatSuccessLog);
    // 放入缓存队列
    push(signKey, JSON.toJSONString(chatSuccessLog));

    // 将chatKey信息放入缓存
    redisTemplate.opsForValue().set(RedisKeyUtils.getChatKey(signKey), JSON.toJSONString(chatUserKey), 5, TimeUnit.MINUTES);
